...
const PriteScoreHistory = () => {
    ...
    // Fetch user's PRITE score history from localStorage
    useEffect(() => {
        const fetchScores = async () => {
            try {
                ...
                setError('Failed to fetch PRITE scores');
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleDelete = async (scoreId) => {
        try {
            ...
            // Filter out the score to delete
            const updatedScores = savedScores.filter(score => score._id !== scoreId);

            // Save back to localStorage
            localStorage.setItem('priteScores', JSON.stringify(updatedScores));
            ...
            setError('Failed to delete score record');
        } finally {
            setLoading(false);
        }
    };
