...
const PriteScoreEntry = ({ user, onComplete, initialData = null, isOnboarding = false }) => {
    ...
    // Save the PRITE score data to localStorage
    const saveScores = async () => {
        try {
            ...
            const finalScoreData = {
                ...scoreData,
                _id: initialData?._id || uuidv4(), // Generate a unique ID if none exists
                ...
                userId: user?._id || 'default-user'
            };

            // Save to localStorage
            const savedScores = JSON.parse(localStorage.getItem('priteScores') || '[]');

            // If editing, replace the existing score
            if (initialData && initialData._id) {
                const index = savedScores.findIndex(score => score._id === initialData._id);
                if (index !== -1) {
                    savedScores[index] = finalScoreData;
                } else {
                    savedScores.push(finalScoreData);
                }
            } else {
                // Otherwise, add as a new score
                savedScores.push(finalScoreData);
            }

            // Save back to localStorage
            localStorage.setItem('priteScores', JSON.stringify(savedScores));

            // Simulate API success
            setTimeout(() => {
                setAlert({
                    type: 'success',
                    message: 'PRITE scores saved successfully (to localStorage)'
                });
